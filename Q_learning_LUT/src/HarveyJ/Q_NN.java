...
public class Q_NN extends AdvancedRobot {
	...
	public static double [][] weights_1 = initialize_weights(35,5);
	public static double [][] bias_1 = initialize_bias(35,1);
	public static double [][] weights_2 = initialize_weights(15,35);
	public static double [][] bias_2 = initialize_bias(15,1);
	public static double [][] weights_3 = initialize_weights(1,15);
	public static double [][] bias_3 = initialize_bias(1,1);
	...
	public static double learning_rate = 0.00009; // 0.00009 3 layer works ok 
	...
	static double error_threshold = 0.05;
	...
	double cum_reward;
	public static double [] win_rate = new double [15000];
		...
		while(true) {
			if (learning == false) {
				...
				for (int j = 0; j< col_num; j++ ) {
					...
					X_train[4][0] = j;
					if(Q_table[index(enemy.x,enemy.y,enemy.distance,enemy.bearing)][j] != 0) {
						Y_train[0][0] = Q_table[index(enemy.x,enemy.y,enemy.distance,enemy.bearing)][j];
				...
				Y_train[0][0] = Q_table[current_state][action];
				System.out.println("the loaded Q table is" + Q_table[current_state][action]);
			...
			//Q_learning starts
			// given the CURRENT STATE , get available actions and choose the one gives max Q value 
			for (int j = 0; j< col_num; j++ ) {
				X_train[0][0] = enemy.x;
				X_train[1][0] = enemy.y;
				X_train[2][0] = enemy.distance;
			...
			if (offpolicy == true) {
				...
				next_q = max_2(q_value_actions);
			}
			else {
				System.out.println("on policy");
				next_q = predict_3(X_train, weights_1,bias_1,weights_2,bias_2, weights_3, bias_3); // need fixing 
			}

			current_q += alpha*(total_reward_per_action+gamma*next_q-current_q);
			Y_train[0][0] = current_q;
			System.out.println("The original weights_1 are " + Arrays.deepToString(weights_1));
			cost = train_3(X_train_previous,Y_train,weights_1,bias_1,weights_2,bias_2,weights_3,bias_3,v1,v2,v3,v4,v5,v6,mu);
			System.out.println("The updated weights_1 are " + Arrays.deepToString(weights_1));
			
			System.out.println("The cost is");
			System.out.println(cost);
			cost = 0.0;
			X_train_previous[0][0] = enemy.x;
			X_train_previous[1][0] = enemy.y;
			X_train_previous[2][0] = enemy.distance;
			X_train_previous[3][0] = enemy.bearing ;
			X_train_previous[4][0] = action ;
			previousAction = action;
			
			current_q = predict_3(X_train_previous, weights_1,bias_1,weights_2,bias_2, weights_3, bias_3);
			total_reward_per_action = 0.0;

			take_action(previousAction);
